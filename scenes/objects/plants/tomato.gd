...
func on_crop_harvesting() -> void:
	var tomato_harvest_scene = tomato_harvest_scene.instantiate() as Node2D
	tomato_harvest_scene.global_position = global_position
	get_parent().add_child(tomato_harvest_scene)
	queue_free()
